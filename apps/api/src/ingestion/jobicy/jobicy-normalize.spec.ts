...
const item: JobicyItem = {
  ...
  jobDescription: '<p>Build &amp; ship <strong>great</strong> UI&hellip;</p>',
...
describe('jobicy normalize', () => {
  ...
  it('prefers the full description and strips HTML/entities', () => {
    expect(normalizeJobicyItem(item, SOURCE_ID).description).toBe('Build & ship great UI');
  });
  ...
  it('falls back to the excerpt when no full description', () => {
    ...
    expect(row.description).toBe('Short excerpt');
  });
