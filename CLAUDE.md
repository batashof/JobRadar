...
- **Current version: 1.19.1** (digest fix: candidates come from the feed's own population instead of `profile_matches`, so an account without a search profile stops getting an empty digest; profile and cached resume scores rank rather than gate, 2026-08-12; keep this line in sync on every bump).
