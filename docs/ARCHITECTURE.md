...
The Bot API side is one module (`apps/api/src/bot`) with one chat link per account (`telegram_accounts`), not one per feature. Features register button handlers by `callback_data` namespace (`n:` planner nudges, `d:` digest, `a:` applying), so the bot module never imports them. The namespaces are also how features avoid importing *each other*: a digest card renders an Apply button as `a:d:<vacancyId>` without knowing that outreach owns the flow behind it. Registering the webhook is a one-off deployment step: `pnpm --filter @jobradar/api bot:webhook` (`--info` / `--delete` to inspect or stop delivery). The live bot is [@JobRadarAppBot](https://t.me/JobRadarAppBot).
